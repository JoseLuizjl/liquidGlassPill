...
float pillSdfAt(vec2 p, vec2 center, vec2 size) {
...
float pillSdf(vec2 p) {
    return pillSdfAt(p, pc.pillCenter, pc.pillSize);
}
...
vec3 liquidGlass(vec2 p, float d) {
    ...
    float edgeWidth = pc.pillSize.y * 0.28;
    float rim = 1.0 - smoothstep(0.0, edgeWidth, insideDistance);
    float rimPower = rim * rim;
    float meniscus = exp(-pow(d + pc.pillSize.y * 0.060, 2.0) / (pc.pillSize.y * pc.pillSize.y * 0.018));
    ...
    float edgeNoise = waveNoise(p * 0.23 + rel * 17.0) * rim * 0.18;
...
void main() {
    ...
    float shadowD = pillSdfAt(p - vec2(0.0, 17.0), pc.pillCenter, pc.pillSize);
    ...
    if (inside > 0.001) {
        ...
        float dots = 0.0;
        for (int i = 0; i < 3; ++i) {
            vec2 dotCenter = pc.pillCenter + vec2(pc.pillSize.x * 0.300 + float(i) * pc.pillSize.y * 0.205, 0.0);
            float dot = 1.0 - smoothstep(pc.pillSize.y * 0.070, pc.pillSize.y * 0.070 + 1.6, length(p - dotCenter));
            dots = max(dots, dot);
        }
